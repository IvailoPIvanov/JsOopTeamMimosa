...
var CanvasGamepad = (function() {
	...
	function setup(config) {
			...
			for (var prop in config) {
				switch (prop) {
					...
					case "buttons":
						...
						if (config[prop].length > buttons_layout.length) {
							buttons = buttons_layout.length - 1;
						}
						buttons_layout = buttons_layout[buttons];
						for (var n = 0; n < buttons + 1; n++) {
							var button = config[prop][n];
							if (button.name) {
								buttons_layout[n].name = button.name;
							}
							if (button.color) {
								buttons_layout[n].color = button.color;
							}
							if (button.key) {
								buttons_layout[n].key = button.key;
							}
		...
		if (!buttons_layout_built) {
			buttons_layout = buttons_layout[buttons];
		};

		if (start) {
			buttons_layout.push(start_button)
		};
		if (select) {
			buttons_layout.push(select_button)
		};
